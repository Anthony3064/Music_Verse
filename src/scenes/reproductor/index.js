...
const ReproductorScene = ({route}) => {
  ...
  const [trackIndex, setTrackIndex] = useState(
    indexOfTrackList(route.params.idParam),
  );
  ...
  const [track, setTrack] = useState(
    route.params.trackListParam[indexOfTrackList(route.params.idParam)],
  );

  const [cargado, setCargado] = useState(route.params.playReproductor);

  useEffect(() => {
    if (
      track !== null &&
      trackIndex !== undefined &&
      route.params.trackListParam != null
    ) {
  ...
  useEffect(() => {
    if (
      millisToMinutesAndSeconds(position * 1000) === '0:00' &&
      track != null
    ) {
      if (replay) {
    ...
    <LinearGradient style={style.containerView} colors={['#34cfeb', '#b8cfd0']}>
      <SafeAreaView>
        ...
        <View style={style.imageContainer}>
          {track == null ? (
            <Image
              ...
              source={require('../../../icons/Fonts/Logo_Music_Verse.jpg')}
            />
          ) : (
            <Image
              style={{width: '80%', borderRadius: 50}}
        ...
        <View style={style.textContainer}>
          {track == null ? (
            <Text style={style.textTitle}>Titulo</Text>
          ) : (
        ...
        <View style={style.sliderContainer}>
          <Text style={{fontSize: 16, color: 'white'}}>
            {millisToMinutesAndSeconds(position * 1000)}
          </Text>
          <Slider
